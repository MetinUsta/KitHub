...
public class Window {
	...
	private void initialize() {
		...
		bookListScrollPane.getVerticalScrollBar()
				.setUI(new ScrollBarColor(textColor, buttonTextColor, backgroundColor));
		bookListScrollPane.getHorizontalScrollBar()
				.setUI(new ScrollBarColor(textColor, buttonTextColor, backgroundColor));
		...
		bookSuggestionsScrollPanel.getVerticalScrollBar()
				.setUI(new ScrollBarColor(textColor, buttonTextColor, backgroundColor));
		...
		librarySelectionScrollPanel.getVerticalScrollBar()
				.setUI(new ScrollBarColor(textColor, buttonTextColor, backgroundColor));
		librarySelectionScrollPanel.getHorizontalScrollBar()
				.setUI(new ScrollBarColor(textColor, buttonTextColor, backgroundColor));
		...
		bookDonationSidemenuLabel
				.setIcon(new ImageIcon(Window.class.getResource("/SystemAssets/Icons/bookDonationIcon.png")));
		...
		libraryDonationScrollPanel.getHorizontalScrollBar()
				.setUI(new ScrollBarColor(textColor, buttonTextColor, backgroundColor));
		...
		openQuoteLabel
				.setIcon(new ImageIcon(Window.class.getResource("/SystemAssets/quoteIcon/colorlessQuoteOpen.png")));
		openQuoteLabel.setBounds(47, 28, 49, 32);
		panel_1.add(openQuoteLabel);

		JLabel closeQuoteLabel = new JLabel("");
		closeQuoteLabel
				.setIcon(new ImageIcon(Window.class.getResource("/SystemAssets/quoteIcon/colorlessQuoteClose.png")));
		...
		oldBooksScrollPane.getVerticalScrollBar()
				.setUI(new ScrollBarColor(textColor, buttonTextColor, backgroundColor));
		oldBooksScrollPane.getHorizontalScrollBar()
				.setUI(new ScrollBarColor(textColor, buttonTextColor, backgroundColor));
		...
		JPanel[] selectionList = { mainPageSidemenuSelection, bookLoanSidemenuSelection, bookDonationSidemenuSelection,
				profileSidemenuSelection };
		...
		mainPageSidemenuBar.addMouseListener(
				new SideBarSelectionHandler(card, contentPanel, selectionList, barList, sideBarSelectionColor,
						sideBarMenuColor, takenBooks, reviewListProfileModel, oldBooksListModel, librarySelectionForm));
		bookLoanSidemenuBar.addMouseListener(
				new SideBarSelectionHandler(card, contentPanel, selectionList, barList, sideBarSelectionColor,
						sideBarMenuColor, takenBooks, reviewListProfileModel, oldBooksListModel, librarySelectionForm));
		bookDonationSidemenuBar.addMouseListener(
				new SideBarSelectionHandler(card, contentPanel, selectionList, barList, sideBarSelectionColor,
						sideBarMenuColor, takenBooks, reviewListProfileModel, oldBooksListModel, librarySelectionForm));
		profileSidemenuBar.addMouseListener(
				new SideBarSelectionHandler(card, contentPanel, selectionList, barList, sideBarSelectionColor,
						sideBarMenuColor, takenBooks, reviewListProfileModel, oldBooksListModel, librarySelectionForm));

		searchBar.addActionListener(new BookSearchListHandler(searchBar, searchTypeComboBox, bookList, list));
		list.addListSelectionListener(new BookInfoListHandler(bookSuggestList, overviewHoverButton, overviewTextLabel,
				bookInfoLabels, reviewListModel));
		bookSuggest.addListSelectionListener(
				new SuggestedBookInfo(overviewHoverButton, overviewTextLabel, bookInfoLabels, reviewListModel));
		commentButton.addActionListener(new NewCommentHandler(textArea, reviewListModel, bookInfoISBNValue));
		bookInfoISBNValue.addPropertyChangeListener(
				new LibraryListHandler(bookInfoISBNValue, libraryList, librarySelectionList));
		librarySelectionList.addListSelectionListener(new libraryListListener(libraryInfoLabels));
		bookLoanButton
				.addActionListener(new LoanBookAction(loggedUser.getUserId(), librarySelectionList, bookInfoISBNValue));
		btnReturnBook.addActionListener(new ReturnBookButtonAction(takenBooksList, takenBooks));
		bookDonateButton.addActionListener(new bookDonationHandler(donateFieldAuthor, donateFieldISBN,
				donateFieldPageCount, donateFieldPublishDate, donateFieldTitle, donateFieldGenres, donateFieldOverview,
				librarySelectionListDonation));
